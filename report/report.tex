...
% Page setup
\usepackage[left=2.5cm, right=2.25cm, top=2.25cm, bottom=2.5cm]{geometry}
...
the distribution of the chain converges to $p$, ensuring that the samples can be used to make unbiased inferences.

When gradient information is available, gradient-based MCMC methods can achieve substantially better sampling efficiency than random-walk methods,
...
We analyse the technical details, discuss the contributions of the work,
and identify limitations and unanswered questions regarding the novel algorithm.
We supplement this analysis with a project comparing RCS's hyperparameter tuning scheme to a Bayesian optimisation approach.
...
We write the target density as $p(\skx) = e^{-\scL(\skx)}/Z$ where $Z = \int e^{-\scL(\skx)} d\skx$ is the normalising constant, and $\scL(\skx) = -\log p(\skx) - \log Z$.
...
MCHMC samples a unit-norm momentum vector every $k \in \nats$ trajectories whereas MCLMC perturbs the momentum frequently during the trajectory \cite{robnik2023microcanonical, robnik2023fluctuation}.
...
Magnetic HMC follows the dynamics $\s[ok]x = F\sku + E \grad U(\skx), \; \s[ok]u = F^T \grad U(\skx) + G \sku$ where $F, E$, and $G$ form an antisymmetric block matrix,
...
Pakman \cite{pakman2025super} explores non-standard dynamics, sampling momentum from the Laplace distribution, yeilding a Hamiltonian (choosing constants equal to 1) $H(\skx, \sku) = - \cos(\skx) + |\sku|$.
Ver Steeg and Galstyan chose a `kinetic' energy $K(\sku) = \frac{d}{2} \log \| \frac{\sku}{d} \|$ \cite{ver2021hamiltonian}.
They argued that in machine learning applications, enforcing physicality is irrelevant \cite{ver2021hamiltonian}, justifying their `non-Newtonian' momentum.
...
There have also been advancements in hyperparameter tuning schemes.
...
To make an algorithm blackbox, $\epsilon$ and $L$, and in some cases a preconditioner matrix to make variability comparable across dimensions \cite{hird2025quantifying},
...
In Section 7, we will compare the tuning scheme for MAMS as described by RCS to a Bayesian optimisation approach.
...
MAMS adds a Metropolis-Hastings accept-reject step to MCHMC to eliminate asymptotic bias.
...
The microcanonical dynamics are approximated using leapfrog integration with step size $\epsilon$ and trajectory length $L$, yielding $n = \lceil L/\epsilon \rceil \in \nats$ steps.
...
Crucially, unlike canonical HMC, the leapfrog integrator for microcanonical dynamics is not volume preserving, necessitating computing a determinant of a Jacobian.
...
\]
The proposal density ratio requires more careful treatment as $\rho$ is not volume preserving.
...
\]
this integral is zero for the canonical case, but accounts for volume compression or expansion in the microcanonical case.
Here, $\sku(s)^T \grad \scL(\skx(s))$ measures the alignment between the log-density and the momentum.
...
To understand this property, consider the behaviour of particles under both systems of differential equations along a trajectory without momentum resampling.
...
In high-probability regions, $p(\skx)$ is high and thus $\scL(\skx) = -\log p(\skx) - \log Z$ is low.
...
Drawing parallels to Hamiltonian dynamics and kinetic energy may confuse readers and practitioners.
...
The approach to tuning MAMS proposed by RCS \cite{robnik2025metropolis} is a sequential optimisation that tunes hyperparameters one at a time across three distinct phases,
...
as each parameter is optimised while holding the other fixed.
When $\epsilon$ is optimised with $L$ held constant, then $L$ is optimised with $\epsilon$ fixed,
the resulting configuration may represent only a local optima.

We hypothesize that jointly exploring the $(L, \epsilon)$ space, using Bayesian optimisation (BayesOpt),
will yield hyperparameter pairs closer to the global optimum, capturing potential interactions that sequential tuning cannot detect.
...
After a chosen number of iterations, the hyperparameters which maximised a carefully chosen objective function are selected and used in all future chains.
...
for the chain's independent hyperparameter adaptation.
If there is variability between hyperparameters found for each chain, this suggests that the RCS tuning method converged to local optima.
...
effectively amortising the initial computational cost.
...
As the repository for the MAMS manuscript is active, it is unclear whether RCS implemented the tuning protocol discussed in their manuscript.
...
The algorithm is initialised with $L = \sqrt{d}$ where $d$ is the dimension of the target density,
...
which could indicate either a broad range of hyperparameters are effective, or that there is instability in the tuning procedure.

Note: as the chain automatically produces hyperparameters for itself.
From the word automatically and the Greek prefix for self, we shall refer to this tuning method as `auto-tuning'.


\subsection{Bayesian Optimisation Method}

Our BayesOpt approach jointly optimises $(L, \epsilon)$ by modelling sampler performance as a function of both hyperparameters simultaneously.
...
\begin{algorithm}
    \caption{Bayesian optimisation for MAMS}
    \begin{algorithmic}%[1]
        ...
        \State Compute maximum $\shR$ across dimensions for each chain, then average over the $C$ chains
        \State Compute mean acceptance rate $\alpha$ for each chain, then average over the $C$ chains
        \State Evaluate objective: $f_t(L_t, \epsilon_t) = \text{ESS}(L_t, \epsilon_t) - P_{\shR}(L_t, \epsilon_t) - P_{\alpha}(L_t, \epsilon_t)$
...
The objective function aims to maximise effective sample size (ESS) while heavily penalising configurations
that lead to non-convergent chains or artificially inflated ESS due to extremely low acceptance rates:
\[
    f(L, \epsilon) = \text{ESS}(L, \epsilon) - P_{\shR}(L, \epsilon) - P_{\alpha}(L, \epsilon).
\]
The convergence penalty $P_{\shR}$ ensures that we only trust ESS estimates from chains that have actually converged according to $\shR$ \cite{gelman1992inference}:
\[
    P_{\shR}(L, \epsilon) = C^2 \cdot N_{\text{tune}} \cdot \max(\shR(L, \epsilon) - 1.0, 0).
\]
...
To avoid this illusion of good performance, we linearly decrease the acceptance rate penalty as acceptance rates increase from 0 to 0.25:
\[
    P_{\alpha}(L, \epsilon) =
    \begin{cases}
...
The maximum $\shR$ across dimensions assesses convergence \cite{gelman1992inference}, with values near 1.0 indicating more likely convergence.
We also track the mean acceptance rate and wall-clock time per chain.
For the 10 validation chains, we present $\shR$, and the mean and standard deviation of ESS, acceptance rate, and time per chain.
...
All dimensions have symmetric structure so diagonal preconditioning offers no advantage over an identity matrix.
The challenge of this target density is achieving adequate mixing between the two modes.

\begin{table}[h]
    \centering
    \caption{Performance comparison on the bimodal Gaussian ($d=10$) mean $\pm$ sd}
    \begin{tabular}{lcccccc}
        \hline
        Method      & $\shR$ & ESS              & Acceptance Rate   & Time/chain (s)    & $L$               & $\epsilon$        \\
        \hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../code/plots/traces_Bimodal Gaussian.png}
    \caption{Trace and scatterplot for the bimodal Gaussian}
    \label{fig:trace_bimodal}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../code/plots/bayesopt_progress_Bimodal Gaussian.png}
    \caption{BayesOpt iterative hyperparameter tuning for the bimodal Gaussian}
    \label{fig:bayesopt_bimodal}
\end{figure}
...
The correlated Gaussian target is centered at the origin with a specific correlation structure:
\[
...
\begin{table}[h]
    \centering
    \caption{Performance comparison on the correlated Gaussian ($d = 50$) mean $\pm$ sd}
    \begin{tabular}{lcccccc}
        \hline
        Method      & $\shR$ & ESS           & Acceptance Rate   & Time/chain (s)    & $L$                & $\epsilon$        \\
        \hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../code/plots/traces_Correlated Gaussian.png}
    \caption{Trace and scatterplot for the correlated Gaussian}
    \label{fig:trace_correlated}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../code/plots/bayesopt_progress_Correlated Gaussian.png}
    \caption{BayesOpt iterative hyperparameter tuning for the correlated Gaussian}
    \label{fig:bayesopt_correlated}
\end{figure}
...
\begin{table}[h]
    \centering
    \caption{Performance comparison on the banana distribution ($d = 2$) mean $\pm$ sd}
    \begin{tabular}{lcccccc}
        \hline
        Method      & $\shR$ & ESS             & Acceptance Rate   & Time/chain (s)     & $L$               & $\epsilon$        \\
        \hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../code/plots/traces_Banana.png}
    \caption{Trace and scatterplot for the banana distribution}
    \label{fig:trace_banana}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../code/plots/bayesopt_progress_Banana.png}
    \caption{BayesOpt iterative hyperparameter tuning}
    \label{fig:bayesopt_banana}
\end{figure}
...
For all three target distributions, both BayesOpt and auto-tuning hyperparameter tuning methods produced similar sampling performances in terms of ESS and $\shR$.
This suggests that for these targets, the $(L, \epsilon)$ hyperparameter space contains regions where hyperparameter configurations perform similarly, and so precise tuning is not required.

For the bimodal Gaussian, both methods fail to achieve adequate convergence, with $\shR > 2$.
The chains never mixed well, and from Figure \ref{fig:bayesopt_bimodal}, the best hyperparameters found by BayesOpt appear to be those which minimised $\shR$.
Neither method found hyperparameters capable of easily switching between modes in the validation chains of length 5000, and this can be seen in the Figure \ref{fig:trace_bimodal}.
...
Auto-tuning was targeting an acceptance rate of 0.90 but realized a rate of $0.988 \pm 0.003$.
...
Notice that $\lceil 1.310 / 1.191 \rceil = 2$ and few leapfrog steps are used.
...
BayesOpt achieved substantially faster wall-clock time (0.742 seconds versus 2.908 seconds),
likely due to having pre-optimised hyperparameters.

Both methods successfully converged for the correlated Gaussian, both with $\shR = 1.003$.
Here again, ESS values are nearly identical (3186 for BayesOpt versus 3223 for auto-tuning).
Again, BayesOpt chose a longer trajectory and larger step size, $24.776$ versus $14.142$, and $3.572$ versus $2.650$ respectively.
The auto-tuning method achieved higher acceptance rates (0.891 versus 0.756).
...
For the banana distribution, both methods achieve excellent convergence with $\shR \approx 1.000$.
...
BayesOpt tuning allows for much greater distances between consecutive samples as seen in Figure \ref{fig:trace_banana} where grey lines connect consecutive samples.
BayesOpt selected an extremely long trajectory $L = 39.602$ with a very small step size $\epsilon = 0.010$.
These are nearly at the bounds of the hyperparameter search space where $L \in [0.5, 40.0]$ and $\epsilon \in [0.01, 4.0]$.
...
These represent different strategies with BayesOpt choosing many small steps along a long trajectory
whereas auto-tuning chose fewer larger steps along a short trajectory.
Interestingly, BayesOpt with the lower trajectory had higher acceptance rates.
...
This is not surprising as the BayesOpt trajectory requires many more computations than those of auto-tuning.
...
For BayesOpt, for none of the three targets do we see the hyperparameters which maximise the objective function occurring near the end of the chain.
Perhaps if BayesOpt was granted more iterations we would see more optimisation rather than exploration.
...
For large-scale applications with many chains, BayesOpt's amortised cost may be lower.
That said, one could also share hyperparameters found through auto-tuning across chains.


Our approach has several limitations.
We have only tuned trajectory length and step size, leaving the inverse mass matrix fixed.
We have not considered the Langevin-like version of MAMS, which has an additional partial momentum resampling hyperparameter.
The BayesOpt search used 20 iterations, which may be insufficient for some targets and hyperparameter search spaces.
The hyperparameter search space used here allows for the step size to be greater than the trajectory length,
...
It was constructed to avoid high ESS from poor mixing and many rejections.
...
We compared the sequential tuning algorithm of RCS to joint tuning using Bayesian optimisation.
The two methods chose drastically different trajectory lengths and step sizes.
Despite discovering substantially different hyperparameter configurations,
both methods achieve similar effective sample sizes and $\shR$ values across all three targets tested.
This means one can choose a tuning method based on convenience and computational budget.
BayesOpt requires a one-time upfront optimisation, and auto-tuning requires adaptation for every chain.
Which is preferable depends on the problem at hand.
