...
# This script is from here: https://github.com/blackjax-devs/blackjax/blob/main/blackjax/adaptation/adjusted_mclmc_adaptation.py
# But I want to be able to access fix_L_first_da
# Blackjax documentation: https://blackjax-devs.github.io/blackjax/_modules/blackjax/adaptation/adjusted_mclmc_adaptation.html#adjusted_mclmc_find_L_and_step_size
# This will ensure that only step size is being modified in the first step of hyperparameter tuning as it says in their paper




# THIS NOW DOES: 10\% step size tuning with L fixed, then 10\% L tuning by itself
def adjusted_mclmc_find_L_and_step_size(
    ...
    rng_key,
    target,
    frac_tune1=0.1, ######## Step size tuning with L fixed since fix_L_first_da is now equal to True
    frac_tune2=0.0, ####### Made this zero
    frac_tune3=0.1, ####### Make this 0.10 so we can get to adjusted_mclmc_make_adaptation_L
    diagonal_preconditioning=False, # TURN THIS TO FALSE SO THE IDENTITY IS KEPT
    params=None,
    max="avg",
    num_windows=1,
    tuning_factor=1.3,
    fix_L_first_da = True ######################################### Added this line
):
    """
    ...
    for i in range(num_windows):
        ...
        (
        ...
        ) = adjusted_mclmc_make_L_step_size_adaptation( ################## Now has fix_L_first_da = True         see below
            kernel=mclmc_kernel,
            dim=dim,
            ...
            fix_L_first_da=fix_L_first_da ####################### Added this line
        )(
