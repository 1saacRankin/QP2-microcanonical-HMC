...
In Section 7, we will compare the tuning scheme for MAMS as described by RCS to a Bayesian optimization approach.
...
\]
this integral is zero for the canonical case as $\rho$ is simplectic but accounts for volume compression or expansion in the microcanonical case.
...
\begin{algorithm}
    ...
    \begin{algorithmic}%[1]
        ...
        \State Evaluate objective: $f_t(L_t, \epsilon_t) = \text{ESS}(L, \epsilon) - P_{\shR}(L_t, \epsilon_t) - P_{\alpha}(L_t, \epsilon_t)$
...
The maximum $\shR$ across dimensions assesses convergence, with values near 1.0 indicating more likely convergence.
...
\begin{table}[h]
    \centering
    \caption{Performance comparison on Bimodal Gaussian ($d=10$) mean $\pm$ sd}
...
\begin{table}[h]
    \centering
    \caption{Performance comparison on Correlated Gaussian ($d = 50$) mean $\pm$ sd}
    \begin{tabular}{lcccccc}
        \hline
        Method      & $\shR$ & ESS           & Acceptance Rate   & Time/chain (s)    & $L$                & $\epsilon$        \\
        \hline
...
\begin{table}[h]
    \centering
    \caption{Performance comparison on Banana Distribution ($d = 2$) mean $\pm$ sd}
    \begin{tabular}{lcccccc}
        \hline
        Method      & $\shR$ & ESS             & Acceptance Rate   & Time/chain (s)     & $L$               & $\epsilon$        \\
        \hline
...
This suggests that for these targets, the $(L, \epsilon)$ hyperparameter space contains regions where hyperparamater configurations perform similarly, and so precise tuning is not required.

For the bimodal Gaussian, both methods fail to achieve adequate convergence, with $\shR > 2$.
The chains never mixed well, and from Figure \ref{fig:bayesopt_bimodal}, the best hyperparamaters appear to be those which minimized $\shR$.
Neither method found hyperparameters capable of easily mode-switching in the validation chain length of 5000, and this can be seen in the Figure \ref{fig:trace_bimodal}.
...
Both methods succesfuly converge for the correlated Gaussian, both with $\shR = 1.003$.
...
We have not consider the Langevin-like version of MAMS, which has an additional partial momentum resampling hyperparameter.
The BayesOpt search used 20 iterations, which may be insufficient for some targets and hyperparameter search space.
...
We compared the sequential tuning algorithm of RCS to joint tuning using Bayesian optimization.
